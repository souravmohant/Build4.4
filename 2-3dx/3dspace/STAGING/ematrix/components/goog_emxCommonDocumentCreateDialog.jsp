...
<%
	 ...
	 //Modified by XPLORIA to hide the CAD Model and CAD Drawing from the UI Selection
	 if ( excludeTypes != null && !excludeTypes.isEmpty()){ 
	     excludeTypes+= ","+"type_CADModel"+","+"type_CADDrawing";
	  }
%>
...
<%
// [Google Custom]: 32670705 - CAD Model Policy - Modified by Sara on 11/05/2018 - Start
StringList sTempList = documentPolicyNames;
if(sTempList.contains("CAD Model") || sTempList.contains("CAD Drawing")){
	StringList sReturnList = new StringList();
	for (int i = 0; i < sTempList.size(); i++) {
		String sPolicyName = (String)sTempList.get(i);
		if(sPolicyName.equals("CAD Model") || sPolicyName.equals("CAD Drawing")){
			sReturnList.add(sPolicyName);
		}
	}
documentPolicyNames = sReturnList;
}
// [Google Custom]: 32670705 - CAD Model Policy - Modified by Sara on 11/05/2018 - End
